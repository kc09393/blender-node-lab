  ...
  steps: [
    {
      ...
      instruction: {
        zh: "把原理化 BSDF 的底色（Base Color）調成黑色、發光強度（Emission Strength）調到 1（這樣畫面只顯示 Math 節點算出來的結果，不受打光影響，方便觀察）。\n\n接著加入雜訊紋理（Noise Texture）跟數學（Math，轉換器 Converter 分類），把雜訊的係數（Fac）接到 Math 的第一個數值，再用合併顏色（Combine Color）把 Math 的結果同時接到 R/G/B，接到發光顏色（Emission Color）。\n\nMath 先保持預設的相加（Add），第二個數值調成 0.15，讓畫面比原本的雜訊稍微亮一點。",
        en: "Set Principled BSDF's Base Color to black and Emission Strength to 1 (so the screen only shows the Math node's raw result, unaffected by lighting).\n\nAdd a Noise Texture and a Math node (Converter category), connect Noise's Fac to Math's first value, then use a Combine Color to feed Math's result into R/G/B, connected to Emission Color.\n\nLeave Math at the default Add, and set its second value to 0.15 so the result looks slightly brighter than the raw noise.",
      },
      check: (graph) =>
    ...
    },
    {
      ...
      instruction: {
        zh: "把運算（Operation）切換成「捨入 Rounding」分類的吸附（Snap），第二個數值改成 0.2。雲霧狀的雜訊會被吸附成 5 個平坦的灰階（0、0.2、0.4、0.6、0.8）。\n\nSnap 把數值強制對齊到指定間隔的整數倍，很適合把連續變化「量化」成幾個明確的階層（也就是常說的 Posterize 效果）。",
        en: "Switch Operation to Snap (in the Rounding category), and set its second value to 0.2. The cloud-like noise snaps into 5 flat gray levels (0, 0.2, 0.4, 0.6, 0.8).\n\nSnap forces a value to the nearest multiple of a given increment, great for quantizing a continuous value into distinct steps (a classic 'posterize' effect).",
      },
    ...
    },
    {
      ...
      instruction: {
        zh: "把運算切換成「比較 Comparison」分類的大於（Greater Than），第二個數值改成 0.5。畫面會變成純黑或純白。\n\n大於這類比較運算永遠只輸出 0 或 1，是做遮罩（哪裡要顯示 A、哪裡要顯示 B）最基本的工具，例如判斷「雜訊值是不是超過某個門檻」。",
        en: "Switch Operation to Greater Than (Comparison category), and set its second value to 0.5. The result becomes pure black-or-white.\n\nComparison operations always output either 0 or 1, making them the fundamental tool for masks (where to show A vs. B), like checking whether a noise value crosses a threshold.",
      },
    ...
    },
    {
      ...
      instruction: {
        zh: "把運算切換成「三角函數 Trigonometric」分類的正弦（Sine）。畫面看起來只是一段平滑的漸層，不太像「週期性」的波紋。\n\n這是因為雜訊的輸出範圍是 0 到 1（換算成弧度只有一小段），正弦函數要跨過好幾圈（大約 0 到 6 以上）才看得出規律的上下震盪。\n\n⚠️ 這是一個很有用的教訓：Sine／Cosine 要先把輸入範圍「放大」（例如接一個 Math 相乘節點乘上 10），才會顯出真正的週期性條紋。",
        en: "Switch Operation to Sine (Trigonometric category). The result just looks like a smooth gradient, not obviously 'periodic'.\n\nThat's because Noise outputs 0 to 1 (only a small slice of a radian), and Sine needs to span several full cycles (roughly 0 to 6 or more) before its up-and-down oscillation becomes visible.\n\n⚠️ This is a useful lesson: Sine/Cosine need their input range 'widened' first (e.g. with a Math Multiply node, scaling by 10 or so) before they show genuine periodic stripes.",
      },
