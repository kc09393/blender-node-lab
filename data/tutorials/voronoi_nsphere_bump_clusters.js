  ...
  steps: [
    {
      ...
      instruction: {
        zh: "加入沃羅諾伊紋理（Voronoi Texture，維持預設的 F1 特徵）跟凹凸（Bump），把沃羅諾伊的距離（Distance）接到凹凸的高度（Height），凹凸的輸出接到原理化 BSDF 的法線（Normal）。\n\nF1 做出來的凹凸是連續的、每個細胞中心到邊界平滑漸變的稜角感。",
        en: "Add a Voronoi Texture (keep the default F1 feature) and a Bump node, connect Voronoi's Distance to Bump's Height, and Bump's output to Principled BSDF's Normal.\n\nF1 produces a continuous, faceted bump that smoothly grades from each cell's center to its boundary.",
      },
      check: (graph) =>
    ...
    },
    {
      ...
      instruction: {
        zh: "把沃羅諾伊的特徵（Feature）切換成 N-球半徑（N-Sphere Radius）。畫面應該會明顯改變：不再是連續的稜角漸變，而是一顆顆分開、圓潤的凸起，很像藤壺或鵝卵石。\n\n因為這個特徵算的是「這個細胞點本身能塞進去、不跟鄰居重疊的最大球半徑」，天生就是「每一格一顆獨立圓球」的概念。",
        en: "Switch Voronoi's Feature to N-Sphere Radius. The result should change noticeably: no longer a continuous faceted gradient, but separate, rounded bumps — like barnacles or pebbles.\n\nBecause this feature computes 'the largest sphere this cell's point fits without overlapping its neighbors', which is inherently 'one independent round bump per cell.'",
      },
    ...
    },
    {
      ...
      instruction: {
        zh: "把沃羅諾伊的縮放（Scale）調高（例如 18 以上）。凸起會變得更小、更密集。\n\n很適合用來做藤壺群聚、青春痘、鉚釘陣列這類「大量重複的小圓凸起」效果。",
        en: "Raise Voronoi's Scale (e.g. above 18). The bumps get smaller and denser.\n\nGreat for barnacle clusters, acne, rivet arrays, or any 'lots of repeating small round bumps' effect.",
      },
    ...
    },
    {
      ...
      instruction: {
        zh: "到邊緣的距離（Distance to Edge）：細胞交界處變亮/凹陷，適合做裂縫、溝紋、鱗片間隙。\n\nN-球半徑（N-Sphere Radius）：每個細胞獨立一顆圓球，適合做藤壺、鵝卵石、疙瘩、鉚釘。\n\n兩者都接凹凸，但選錯特徵做出來的效果會完全不像預期，先想清楚「要連續的溝紋」還是「要分開的圓凸起」，再決定選哪個。",
        en: "Distance to Edge: brightens/recesses at cell boundaries — good for cracks, grooves, gaps between scales.\n\nN-Sphere Radius: one independent round bump per cell — good for barnacles, pebbles, warts, rivets.\n\nBoth feed into Bump, but picking the wrong feature gives a result nothing like what you wanted — decide 'continuous grooves' vs. 'separate round bumps' first, then pick accordingly.",
      },
      check: (graph) => anyNodeParamMatches(graph, "texture_voronoi", "feature", (v) => v === "n_sphere_radius"),
    },
  ],
