  ...
  steps: [
    {
      ...
      instruction: {
        zh: "加入雜訊紋理（Noise Texture）跟映射範圍（Map Range，轉換器 Converter 分類），把雜訊的係數（Fac）接到映射範圍的數值（Value），再把映射範圍的結果接到原理化 BSDF 的粗糙度（Roughness）。\n\n預設是線性（Linear）插值，效果等同於原封不動地換算範圍。",
        en: "Add a Noise Texture and a Map Range (Converter category), connect Noise's Fac to Map Range's Value, then connect Map Range's result to Principled BSDF's Roughness.\n\nThe default Linear interpolation just rescales the range as-is.",
      },
      check: (graph) =>
    ...
    },
    {
      ...
      instruction: {
        zh: "把插值方式（Interpolation）切換成階梯（Stepped），階數（Steps）設成 4。粗糙度的變化會被強制分成 4 個明確的階層，不再是連續漸變。\n\n反光的清晰程度會出現「一塊一塊」跳動的感覺，而不是平滑過渡。",
        en: "Switch Interpolation to Stepped, and set Steps to 4. Roughness variation gets forced into 4 distinct levels instead of a smooth gradient.\n\nThe reflection sharpness will jump between a few discrete looks rather than blending continuously.",
      },
    ...
    },
    {
      ...
      instruction: {
        zh: "把插值方式切換成更平滑（Smootherstep）。\n\n這是平滑（Smoothstep）的進階版——兩端變化速度趨近於零，而且連「變化速度改變的速度」也趨近於零，過渡感又更柔和一階，兩者都比線性更「有機」，常用來讓數值過渡不要那麼死板生硬。",
        en: "Switch Interpolation to Smootherstep.\n\nThis is Smoothstep's upgraded sibling — velocity approaches zero at both ends, and even the rate of change of that velocity approaches zero too, making the transition one notch softer still. Both are more organic-feeling than a flat linear ramp.",
      },
    ...
    },
    {
      ...
      instruction: {
        zh: "打開夾值（Clamp）開關。\n\n這會把最終結果強制鎖在目標最小（To Min）與目標最大（To Max）之間——如果上游數值意外超出你預期的範圍（例如疊加了好幾層運算後跑到 1.2），Clamp 能避免它繼續往下傳、把後面的材質參數弄壞。",
        en: "Turn on the Clamp toggle.\n\nThis locks the final result between To Min and To Max — if an upstream value unexpectedly exceeds your intended range (e.g. drifting to 1.2 after several layers of math), Clamp stops it from propagating further and corrupting downstream material parameters.",
      },
