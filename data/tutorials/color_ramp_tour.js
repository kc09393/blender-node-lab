  ...
  steps: [
    {
      ...
      instruction: {
        zh: "加入雜訊紋理（Noise Texture）跟顏色漸變（Color Ramp，轉換器 Converter 分類），把雜訊紋理的係數（Fac）接到顏色漸變的係數（Fac），再把顏色漸變的顏色（Color）接到原理化 BSDF 的底色（Base Color）。\n\n預設是黑到白的漸層——這就是 Color Ramp 最基本的功能：把一個數值換算成顏色。",
        en: "Add a Noise Texture and a Color Ramp (Converter category). Connect Noise's Fac to Color Ramp's Fac, then Color Ramp's Color to Principled BSDF's Base Color.\n\nThe default is a black-to-white gradient — Color Ramp's most basic job: turning a value into a color.",
      },
      check: (graph) =>
    ...
    },
    {
      ...
      instruction: {
        zh: "點漸層條下方的「+」新增一個停駐點，把它拖到中間位置（約 0.5），改成第三種顏色。現在漸層從兩色變成三色。\n\n停駐點（Stop）就是漸層上的「顏色關鍵格」，越多停駐點能做出越複雜的配色。",
        en: "Click the '+' below the gradient bar to add a stop, drag it to the middle (around 0.5), and change it to a third color. The gradient now goes from two colors to three.\n\nStops are the gradient's color keyframes; more stops let you build more complex color schemes.",
      },
    ...
    },
    {
      ...
      instruction: {
        zh: "把色彩空間（Color Mode）切換成 HSV。\n\n差別在於「插值是在哪個色彩空間裡算的」：RGB 模式跨色相過渡時，中間常會不小心經過灰色/濁色；HSV 模式繞著色相環走，過渡通常更鮮豔、更符合直覺（例如紅到藍會經過紫色，而不是變灰）。",
        en: "Switch Color Mode to HSV.\n\nThe difference is which color space the interpolation happens in: RGB mode can accidentally pass through gray/muddy colors across a hue transition; HSV mode travels around the hue wheel, usually giving a more vivid, intuitive transition (e.g. red to blue passes through purple instead of gray).",
      },
    ...
    },
    {
      ...
      instruction: {
        zh: "把插值方式（Interpolation）切換成常量（Constant）。\n\n前面幾種模式都是「平滑過渡」，Constant 完全不過渡——每個停駐點的顏色會硬切換到下一個停駐點，做出邊界分明的色塊，很適合卡通風格或分區上色。",
        en: "Switch Interpolation to Constant.\n\nThe earlier modes all blend smoothly; Constant doesn't blend at all — each stop's color holds until it hard-cuts to the next one, giving sharp-edged color blocks. Great for toon styling or zoned coloring.",
      },
